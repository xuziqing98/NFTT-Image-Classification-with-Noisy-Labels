...
class cifar_dataset(Dataset):
    ...
    def __delete__(self, index_list):
        self.train_data = np.delete(self.train_data,index_list,0)
        self.noise_label = np.delete(np.array(self.noise_label), index_list, 0).tolist()
        self.clean_label = np.delete(np.array(self.clean_label), index_list, 0).tolist()
...
class cifar_dataloader():
    ...
    def run(self, mode):
        if mode == 'train':
            ...
            return train_dataset, trainloader, np.asarray(train_dataset.noise_label), np.asarray(train_dataset.clean_label)
